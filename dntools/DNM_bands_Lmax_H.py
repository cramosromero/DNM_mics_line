...
plt.xlabel('Time [s]')
plt.tight_layout()
plt.savefig(ffolder+'/'+identifier+'_'+'allChan_Press'+'Flyby{}'.format(event)+'.jpg', format='jpg',dpi=900)
# # plt.show()
...
plt.xlabel('Time [s]')
plt.tight_layout()
plt.savefig(ffolder+'/'+identifier+'_'+'allFlybys_Press'+'Chan{}'.format(event)+'.jpg', format='jpg',dpi=900)
# # plt.show()
...
DATA_PSD_events, freq = FreqTools.calc_PSDs(DATA_raw_events, Fs, Ndft, WINDOW)#, NOVERLAP)
freq = np.array(freq)
...
fig, (ax0) = plt.subplots(figsize=(6,3))
f, t, Sxx = ss.spectrogram(x, nperseg=(2**17)//8, fs=Fs, nfft=2**18)

y_lim =[70,500]
#RdBu_r
plt.pcolormesh(t, f, 10*np.log10(Sxx/(20e-6)**2),vmin=10,vmax=50,cmap='viridis',shading='auto') # vmin=10,vmax=60 
...
plt.tight_layout()
plt.savefig(ffolder+'/'+identifier+'_'+'SpectA'+'.jpg', format='jpg',dpi=900)
# plt.show()
...
plt.tight_layout()
plt.savefig(ffolder+'/'+identifier+'_'+'SpectB'+'.jpg', format='jpg',dpi=900)
plt.show()
...
if band_selection == 'my_band':
    ...
    del_f = freqs_selected[1]-freqs_selected[0] #delta F for SIEMENS CORRECTION.
    p_selected = DATA_PSD_events[:,C,:] 
    p2_mean_selected = np.sum(p_selected,axis=1) #array [event, channles] 
    SPL_event_mic = 10*np.log10(p2_mean_selected/p_ref**2)+20*np.log10(del_f) #SIEMENS CORRECTION due discrete contributors
    ...
    band_name = "BPF narrow band: "+str(fd_)+" to "+str(fu_)+" Hz"
...
#####################
    ...
    """IF I select wichever band"""
    fraction = 3
    DATA_SPLoverall, DATA_SPLs, freq = FreqTools.calc_SPLs(DATA_raw_events,Fs, fraction, limits=[20, 20000], show=0)
    ...
    if len(Band)==1:
        band_name = "BPF $1^{st}$ harmonic (1/3-octave band: " +str(Band[0]) +" Hz)"
    else:
        band_name = "OASPL (1/3-octave band: "+str(Band[0])+' to '+str(Band[-1]) +" Hz)"
...
# if Count =='nw' : ##flip the aray if the drone flyes up-wind (for the order in the Sphere)
...
for pp in range(len(DI_SPL_lev)):
    form=next(lc)
    ax.plot(theta, np.array(DI_SPL_lev[pp]), dfor, label=Band_names[pp],
...
# plt.minorticks_on()
plt.savefig(ffolder+'/'+identifier+'_'+'Dir.jpg', format='jpg',dpi=900)
...
pickle.dump( C_G_JAST, open(ffolder+'/'+identifier+"_Leq.p", "wb" ) )
